...
### load the library
pkgs = c('glmnet','caret','data.table','pROC','argparse','xlsx')
...
parser$add_argument('-l', '--label', dest='label', metavar='FILE', nargs='+',
...
data_pre <- function(data_input,label_input){
  ...
  input_dat = read.csv(data_input[1],header = T,
                       row.names = 1,stringsAsFactors = FALSE,check.names = FALSE,sep = '\t',comment.char = "#")
  if (length(data_input) > 1) {
    for (i in 2:length(data_input)) {
      input_dat = cbind(input_dat, read.csv(data_input[i],header = T,
                                            row.names = 1,stringsAsFactors = FALSE,check.names = FALSE,sep = '\t',comment.char = "#"))
    }
  }
  
  input_dat = as.data.frame(t(input_dat))
  input_dat = input_dat[,!apply(is.na(input_dat),2,any)]
  input_dat = input_dat[,!apply(input_dat,2,function(x) all(x==0))]
  ...
  if (length(label_input) > 1) {
    for (i in 2:length(label_input)) {
      lable_info = rbind(lable_info, fread(label_input[i],
...
Select_par <- function(data_input,label_input, output_path,n_times = 50,my_step = 0.01){
  ...
  if(missing(output_path)){
    ...
    dir.exists(output_path) || dir.create(output_path)
    my_path = output_path
  }
  ...
  for(i in 1:n_times){
    ...
    for(i in i_set){
      fit_cv4 = cv.glmnet(train_x, train_y, family="binomial",
                          alpha=i,type.measure = "auc",standardize = F) # run the model
  ...
  select_pra = subset(output, error_mean == max(error_mean))
...
CV_threshold <- function(data_input,label_input,output_path,best_alpha = 0.7,best_lambda=0.01){
  ...
  dir.exists(paste0(my_path,'/Cross_Validation')) || 
    dir.create(paste0(my_path,'/Cross_Validation'))
  my_path = paste0(my_path,'/Cross_Validation')
  data_preoutput_output = data_pre(data_input,label_input) #data input
  train_set_input = data_preoutput_output$train_set
  # train_set_input = train_set_input[,c(as.character(feature_sele_350),'label')]
  
  pdf_path = paste0(my_path,'/CV_auc_plot.pdf') # pdf path
  ...
  auc_model = c()
  for(i in 0:my_step){
    set_myseed = as.integer(paste(201818,i,sep = ""))
    set.seed(set_myseed)
    # cv_Folds = createFolds(1:dim(train_set_input)[1],k=2)
    cv_Folds = createFolds(train_set_input$label,k=2)
    par(mfrow = c(2,1))
    
    for(k in 1:length(cv_Folds))
    { 
      loop_count = loop_count+1
      train_dat = train_set_input[-cv_Folds[[k]],]
      vali_dat = train_set_input[cv_Folds[[k]],]
      ...
      train_dat$label = factor(train_dat$label)
      set.seed(8)
      # fit_bi = randomForest(label ~.,data=train_dat,mtry = 47, ntree = 500)
      # fit_bi = randomForest(label ~.,data=train_dat)
      # fit_bi = randomForest(label ~.,data=train_dat,ntree = 1000)
      # fit_bi = randomForest(label ~.,data=train_dat, ntree = 1000)
      # pred_vali = predict(fit_bi,newdata=vali_x,type='prob')[,1]
      fit_bi = glmnet(train_x, train_y, family="binomial",
                      alpha=best_alpha, lambda = best_lambda,standardize = F) # model
      pred_vali= as.data.frame(predict(fit_bi,
                                       vali_x,type ='response'))$s0 #   predict validation data
      # save model
      # add score and label
      add_score = append(add_score,as.numeric(pred_vali))
      add_label = append(add_label,vali_y)
      add_count = append(add_count,rep(loop_count,length(vali_y)))
      
      my_model_path = paste0(my_path,"/CV_glmnet_model",loop_count,".rds")
      ...
      best_thresholds = round(as.numeric(rownames(vali_th$ci$specificity)),3)[1]
      AUC = vali_th$auc[1]
  ...
  # mean、sd of se sp auc 
  mean_se = mean(se)
  sd_se = sd(se)
  mean_sp = mean(sp)
  sd_sp = sd(sp)
  mean_auc = mean(auc_model)
  sd_auc = sd(auc_model)
  calculate_mean_sd = data.frame(mean_se=mean_se,sd_se=sd_se,mean_sp=mean_sp,sd_sp=sd_sp,mean_auc=mean_auc,sd_auc=sd_auc)
  calculate_mean_sd = format(calculate_mean_sd,digits=4)
  # data
  out_se_sp_cutoff_auc = data.frame(se,sp,best_cutoff,auc_model)
  out_se_sp_cutoff_auc = format(out_se_sp_cutoff_auc,digits=4)
  CV_se_sp_cutoff_auc = list(best_alpha=best_alpha,best_lambda=best_lambda,cal=calculate_mean_sd,count_se_sp=out_se_sp_cutoff_auc)
  txt_path = paste0(my_path,'/CV_se_sp_cutoff_auc.txt')
  capture.output(CV_se_sp_cutoff_auc, file = txt_path)
  
  #
  add_score_label = data.frame(score=add_score,label=add_label,Group = add_count)
  write.csv(add_score_label,paste0(my_path,'/CVtotal_score_label.csv'))
  
  png(paste0(my_path,'/CVtotal_auc.png'),width = 436,height = 438)
  rocobj <- plot.roc(add_score_label$label, add_score_label$score,
  ...
  text(40,45,sprintf('AUC: %.1f%%',as.numeric(CV_se_sp_cutoff_auc$cal$mean_auc)*100),cex=1.5)
...
Boostrap_testModel <- function(data_input,label_input,output_path,best_alpha = 0.7,best_lambda=0.01){
  ...
  dir.exists(paste0(my_path,'/Bootstrapping')) || 
    dir.create(paste0(my_path,'/Bootstrapping'))
  my_path = paste0(my_path,'/Bootstrapping')
  #
  data_preoutput_output = data_pre(data_input,label_input) #  call the function : data_pre
  ...
  random_sampling <- function(train_x,train_y,best_alpha,best_lambda,n){
    ...
    for(i in seq(n)){
      ...
      X_test = train_x[sub1,]
      y_test = train_y[sub1]
      fit_model = glmnet(X_train, y_train, family="binomial",alpha=best_alpha, lambda = best_lambda,standardize = F) # model
      ...
      # save score
      write.xlsx(pred, sprintf("%s/boostrap_score.xlsx", my_path), 
                row.names=T, sheetName=paste0('score_random_sampling_',i),append = T)
      # save model
    ...
    write.xlsx(random_sampling_data, sprintf("%s/boostrap_score.xlsx", my_path), 
                row.names=F, sheetName="random_sampling_total_info",append = T)
  ...
  random_sampling(train_x,train_y,best_alpha,best_lambda,3)
}
...
Train_model <- function(data_input,label_input,output_path,best_alpha = 0.7,best_lambda=0.01){
  ...
  train_x = as.matrix(subset(train_set,select = -label)) # train set
  train_y = train_set$label
  fit_model = glmnet(train_x, train_y, family="binomial",alpha=best_alpha, lambda = best_lambda,standardize = F) # model

  pred_train_x= as.data.frame(predict(fit_model,train_x,type ='response'))$s0
  # calculate auc、 cutoff 
  my_level = unique(train_y)
  train_th = roc(train_y, pred_train_x,levels = my_level,ci=TRUE, 
                 of="thresholds",thresholds="best") # best thresholds
  # best cutoff of model
  best_thresholds = round(as.numeric(rownames(train_th$ci$specificity)),3)
  fit_model$cutoff = best_thresholds
  # mean、sd of model
  fit_model$mean = data_preoutput_output$mean
  fit_model$sd = data_preoutput_output$sd
  # save model
  my_model_path = paste0(my_path,"/final_glmnet_model.rds")
  saveRDS(fit_model, file=my_model_path)
  
  AUC = train_th$auc[1]
  train_thr_auc = data.frame(best_thresholds,AUC)
  train_thr_auc = format(train_thr_auc,digits=4)
  ...
  train_thr_se_sp = format(train_thr_se_sp,digits=4)
  merge_output_dat = list(train_thr_auc,train_thr_se_sp)
  txt_path = paste0(my_path,'/final_glmnet_model_auc_thresholds.txt')
  # save auc、cutoff 
  capture.output(merge_output_dat, file = txt_path)
  # save auc img 
  png_auc = paste0(my_path,'/final_glmnet_model_ROC.png')
  png(png_auc,width = 436,height = 438)
...
### predict new data
Predict_function <-function(data_input,my_model_path,output_path){
  if(missing(output_path)){
    my_path = getwd()
  }else{
    dir.exists(output_path) || dir.create(output_path)
  ...
  my_model = readRDS(my_model_path)# load model
  # coefficient  
  model_useRegion = rownames(as.data.frame(as.matrix(my_model$beta)))
  model_cutoff = my_model$cutoff
  model_mean = my_model$mean
  model_sd = my_model$sd
  # new data
  input_dat = read.csv(data_input[i],header = T,
                       row.names = 1,stringsAsFactors = FALSE,check.names = FALSE,sep = '\t',comment.char = "#")
  if (length(data_input) > 1) {
    for (i in 2:length(data_input)) {
      input_dat = cbind(input_dat, read.csv(data_input[i],header = T,
  ...
  # scale
  input_dat = as.data.frame(t(input_dat))
  input_dat = input_dat[,names(model_mean)] # match index
  input_dat = scale(input_dat,center = model_mean,scale=model_sd)
  # input_dat = as.data.frame(input_dat)
  input_dat_model_useRegion = input_dat[,model_useRegion]
    
  pred_score = as.data.frame(predict(my_model,input_dat_model_useRegion,type ='response'))
  colnames(pred_score) = c("score")
  pred_score$predict_class = ifelse(pred_score$score> model_cutoff,'Malignant','Benign')
  write.csv(pred_score, paste0(my_path,'/score.csv'))
  # print(model_cutoff)
  cat("output file: ", sprintf("%s", paste0(my_path,'/score.csv')), " sucess!\n")
}




# outp = select_par(args$sample,args$label,args$outpath,n_times = 10,my_step = 0.1)
# print(outp)
# CV_threshold(args$sample,args$label,args$outpath,best_alpha = outp$best_alpha,best_lambda=outp$best_lambda)
# 
# data_input = '/home/binyang_ni/test/CR_classify/input_dat_scale_201709.csv'
# label_input = '/home/binyang_ni/test/CR_classify/label_full.tsv'

data_input = args$sample
label_input = args$label
output_path = args$outpath
# my_model_path = paste0(args$outpath,"/final_glmnet_model.rds")
my_model_path = args$model
# sprintf('Predict:%s',!args$Predict)
# sprintf('Train:%s',!args$Train)

if(args$Train){
  
  cat(' parameter tuning...')
  outp = Select_par(data_input,label_input,output_path,n_times = 30,my_step = 0.1)

  cat('done!\n cross validation...')
  CV_threshold(data_input,label_input,output_path,best_alpha = outp$best_alpha,best_lambda=outp$best_lambda)

  Boostrap_testModel(data_input,label_input,output_path,best_alpha = outp$best_alpha,best_lambda=outp$best_lambda)

  cat('done!\n training final model...')
  Train_model(data_input,label_input,output_path,best_alpha = outp$best_alpha,best_lambda=outp$best_lambda)

  cat('done!\n')
  
}else if(args$Predict){
  Predict_function(data_input,my_model_path,output_path)
