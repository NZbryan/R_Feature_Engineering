...
select_par <- function(data_input,label_input, set_fold = 3, n_times = 50,my_step = 0.01){
  ...
  # n_times = 50
  # i_set = seq(0,1,0.01)
  pdf_path = paste0(my_path,'/auc_choose_alpha_lambda.pdf') # pdf path
  pdf(pdf_path)
  ...
  for(i in 1:n_times){
    ...
    cv_df = c()
    for(i in i_set){
      fit_cv4 = cv.glmnet(train_x, train_y, family="binomial",nfolds=5,
                          alpha=i,type.measure = "auc") # run the model
      #plot
      plot(fit_cv4)
      max_auc = fit_cv4$cvm[fit_cv4$lambda==fit_cv4$lambda.min]
      x_l = log(fit_cv4$lambda.min)
      y_1 = max_auc-0.05
      text(x_l,y_1,sprintf("max auc: %2f",max_auc),col = "blue")
      ...
      cv_value_error = append(cv_value_error,max(fit_cv4$cvm)) # Statistical error
      cv_df = append(cv_df,
  ...
  colnames(output) = c('alpha','auc_accumulation','auc_mean',
                       'lambda_accumulation','lambda_mean','df_mean') # rename  the output data.frame
  select_pra = subset(output, auc_mean == min(auc_mean))
...
CV_threshold <- function(data_input,label_input,best_alpha = 0.7,best_lambda=0.01){
  ...
  for(i in 0:my_step){
    ...
    set.seed(set_myseed)
    cv_Folds = createFolds(1:dim(train_set)[1],k=2)
    par(mfrow = c(3,1))
...
#
predict_function <-function(test_df,my_model,my_cutoff = 0.378){
...
data_input_path = "D:/R_workspace/glmnet_project/model_cv/test_cv_unifNorm.csv"
label_input_path = "D:/R_workspace/glmnet_project/model_cv/label_cv.txt"
model_path = paste0(dirname(data_input_path),"/glmnet_model.rds")

# data1212 = data_pre(data_input_path,label_input_path)
oup = select_par(data_input_path,label_input_path,n_times = 20,my_step = 0.1)
best_alpha = oup$best_alpha
best_lambda = oup$best_lambda

CV_threshold(data_input_path,label_input_path,best_alpha = best_alpha,best_lambda=best_lambda)
...
# my_model = readRDS(model_path)# load model
