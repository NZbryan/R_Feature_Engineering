...
CV_threshold <- function(data_input,label_input,best_alpha = 0.7,best_lambda=0.01){
  ...
  outp_mean_thresholds = paste0('mean of thresholds','(',times_count,' times)',': ',mean_thresholds)
  #sprintf('mean of thresholds(%i times): %f' ,times_count,mean_thresholds)
  return(outp_mean_thresholds)
}
...
# main() --------------------------------------------------------------------------------------------------------------
run_main <-function(data_input,label_input){
  if(nchar(dirname(data_input))==1){
    my_path = getwd()
  }else{
    my_path = dirname(data_input)
  ...
  #
  select_par_output = select_par(data_input,label_input,n_times=3,my_step=0.1) # call select_par()
  best_alpha = select_par_output$best_alpha
  best_lambda = select_par_output$best_lambda
  
  mean_thresholds = CV_threshold(data_input,label_input,
                                      best_alpha = best_alpha,best_lambda=best_lambda)# call CV_threshold()
  train_model(data_input,label_input,best_alpha = best_alpha,best_lambda=best_lambda)# call train_model()
  # save txt: best_alpha 、best_lambda、mean_thresholds
  txt_path = paste0(my_path,'/auc_thresholds.txt')
  merge_output_dat = list(best_alpha,best_lambda,mean_thresholds)
  capture.output(merge_output_dat, file = txt_path)
  # get the test set
  output_new = data_pre(data_input,label_input)
  test_data = output_new$test_set
  test_x = as.matrix(subset(test_data,select = -label)) # train set
  test_y = test_data$label
  # predict the test set
  model_path = paste0(my_path,"/glmnet_model.rds")
  pred_score = predict_function(test_x,model_path,my_cutoff = 0.43)
...
data_input_path = "/home/binyang_ni/R/R_data/glm_project_data/data_unifNorm2.csv"
label_input_path = "/home/binyang_ni/R/R_data/glm_project_data/label_new.txt"

select_par(data_input_path,label_input_path,n_times = 50,my_step = 0.01)
